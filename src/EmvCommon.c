...
int GetEmvTrackData(u8 * pTrackBuf)
{
	...
	if( (PosCom.stTrans.Trans_id != POS_SALE) && (PosCom.stTrans.Trans_id != POS_QUE) )
	{
		iRet = SetIssuerName();
		if(iRet != 0)
		{
...
int EmvSaveData(void)
{
	...
	//  
	if(!PosCom.stTrans.IccOnline && (PosCom.stTrans.Trans_id == POS_SALE))
	{
...
SELECT_NEXT:
	...
	if (PosCom.stTrans.IccOnline) { //online transaction
		...
		//PayPass_GetOutcome_Api(&outcome); //运行这行崩溃
        ...
        if (aIfMoveCard == 1) {
            ...
            if (iTemp == 0) {
                ...
                while (1) {
                    ...
                    iTemp = PiccCheck(gCtrlParam.SupportPICC);
                    if (iTemp == 0) {
                        break;
                    }
                }
...
int PaywaveTransComplete()
{
	...
	if (1 == needIssuer) {
		...
		while(!TimerCheck_Api(timerid, 60 * 1000)){ 
			if(PiccCheck(gCtrlParam.SupportPICC) == 0){	
...
#if (defined(WIN32))  // only used for compiling for simulator******start*******
//comlib init
...
#if (defined(WIN32) || defined (LIB_MIR_UNEXIST))
//MIR
