...
int GetCard(u8 mode, u8 type)
{
	...
	while(1)
	{
		...
		case MASK_INCARDNO_PICC:	
			Common_SetIcCardType_Api(gCtrlParam.SupportPICC, 0);
		...
		case MASK_INCARDNO_MAGCARD:							 
			...
			{
				ScrDisp_Api(LINE2, 0, "Swipe Card Error", LDISP);
				ScrDisp_Api(LINE3, 0, "Swipe Card Again-->", LDISP);
				ScrDisp_Api(LINE4, 0, "Cancel To Exit", LDISP);
				redisp = 0;
				break;
			}
			...
			ret = DispCardNumber(PosCom.stTrans.MainAcc, strlen(PosCom.stTrans.MainAcc) );
			return ret;
